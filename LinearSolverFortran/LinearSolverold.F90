...
contains
  ...
  subroutine jacobiPointSolveOMP
    ...
    real*8             ::  matLHSTemp(1:nVars,1:nVars),solm1Temp(1:nVars)
       ...
       do k=kl_bnd+nguard*iins3d,ku_bnd-nguard*iins3d
          ...
          do j=jl_bnd+nguard,ju_bnd-nguard
             ...
             do i=il_bnd+nguard,iu_bnd-nguard
                ...
                ! i-1,j,k
                !
                matLHSTemp(1:nVars,1:nVars) = matLHS(2,i,j,k,1:nVars,1:nVars) ! matLHSTemp
                solm1Temp(1:nVars) = solm1(i-1,j,k,1:nVars)                   ! solm1Temp
                !
                call matrixMult(matLHSTemp(1:nVars,1:nVars),solm1Temp(1:nVars),vec_tmp(1:nVars),nVars)
                rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp(1:nVars)
                !
                ! i+1,j,k
                !
                matLHSTemp(1:nVars,1:nVars) = matLHS(3,i,j,k,1:nVars,1:nVars) ! matLHSTemp
                solm1Temp(1:nVars) = solm1(i+1,j,k,1:nVars)                   ! solm1Temp
                !              
                call matrixMult(matLHSTemp(1:nVars,1:nVars),solm1Temp(1:nVars),vec_tmp(1:nVars),nVars)
                rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp(1:nVars)
                ...
                ! i,j-1,k
                !
                matLHSTemp(1:nVars,1:nVars) = matLHS(4,i,j,k,1:nVars,1:nVars) ! matLHSTemp
                solm1Temp(1:nVars) = solm1(i,j-1,k,1:nVars)                   ! solm1Temp     
                !           
                call matrixMult(matLHSTemp(1:nVars,1:nVars),solm1Temp(1:nVars),vec_tmp(1:nVars),nVars)
                rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp(1:nVars)
                !
                ! i,j+1,k
                !
                matLHSTemp(1:nVars,1:nVars) = matLHS(5,i,j,k,1:nVars,1:nVars) ! matLHSTemp
                solm1Temp(1:nVars) = solm1(i,j+1,k,1:nVars)                   ! solm1Temp     
                ! 
                call matrixMult(matLHSTemp(1:nVars,1:nVars),solm1Temp(1:nVars),vec_tmp(1:nVars),nVars)
                rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp(1:nVars)
                ...
                ! i,j,k-1
                !
                matLHSTemp(1:nVars,1:nVars) = matLHS(6,i,j,k,1:nVars,1:nVars) ! matLHSTemp
                solm1Temp(1:nVars) = solm1(i,j,k-1,1:nVars)                   ! solm1Temp     
                !                 
                call matrixMult(matLHSTemp(1:nVars,1:nVars),solm1Temp(1:nVars),vec_tmp(1:nVars),nVars)
                rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp(1:nVars)
                ! i,j,k+1
                !
                matLHSTemp(1:nVars,1:nVars) = matLHS(7,i,j,k,1:nVars,1:nVars) ! matLHSTemp
                solm1Temp(1:nVars) = solm1(i,j,k+1,1:nVars)                   ! solm1Temp     
                !                    
                call matrixMult(matLHSTemp(1:nVars,1:nVars),solm1Temp(1:nVars),vec_tmp(1:nVars),nVars)
                rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp(1:nVars)                      
#endif
                !
                call matrixMult(Dinv(1:nVars,1:nVars),rhstmp2(1:nVars),sol_tmp(1:nVars),nVars)
                !
  ...
  !
  subroutine matrixMult(matLHSTemp,solm1Temp,vec_tmp,nVars) 
    use omp_lib
    implicit none
    integer  :: nVars
    Real(8)  :: matLHSTemp(nVars,nVars),solm1Temp(nVars),vec_tmp(nVars) 
    integer  :: i,j
    Real*8   :: s
    !$omp parallel &
    !$omp shared ( matLHSTemp, solm1Temp, vec_tmp, s) &
    !$omp private ( i, j)
    ...
    do i = 1, nVars
       s = 0.0
       do j = 1, nVars
         s = s + matLHSTemp(i,j) * solm1Temp(j)
       end do
       vec_tmp(i) = s
    end do
  !$omp end do
