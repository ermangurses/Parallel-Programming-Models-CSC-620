...
contains
         ...
         !$omp& private (i,j,k)                            
          do j=jl_bnd+nguard,ju_bnd-nguard
             ...
             do i=il_bnd+nguard,iu_bnd-nguard
                ...
                !
                ! i-1,j,k
                vec_tmp2(1:nVars)=matmul(matLHS(2,i,j,k,1:nVars,1:nVars),solm1(i-1,j,k,1:nVars))
                !rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp2(1:nVars)
                !
                ! i+1,j,k
                vec_tmp3(1:nVars)=matmul(matLHS(3,i,j,k,1:nVars,1:nVars),solm1(i+1,j,k,1:nVars))
                !rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp3(1:nVars)
                !
                ! i,j-1,k
                vec_tmp4(1:nVars)=matmul(matLHS(4,i,j,k,1:nVars,1:nVars),solm1(i,j-1,k,1:nVars))
                !rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp4(1:nVars)
                !
                ! i,j+1,k
                vec_tmp5(1:nVars)=matmul(matLHS(5,i,j,k,1:nVars,1:nVars),solm1(i,j+1,k,1:nVars))
                !rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp5(1:nVars)
                !
#if (iins3d==1)
                ! i,j,k-1
                vec_tmp6(1:nVars)=matmul(matLHS(6,i,j,k,1:nVars,1:nVars),solm1(i,j,k-1,1:nVars))
                !rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp6(1:nVars)
                ! i,j,k+1
                vec_tmp7(1:nVars)=matmul(matLHS(7,i,j,k,1:nVars,1:nVars),solm1(i,j,k+1,1:nVars))
                !rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp7(1:nVars)                      
#endif
                !
                rhstmp2(1:nVars)=rhstmp2(1:nVars)-vec_tmp2(1:nVars)&
                                                 -vec_tmp3(1:nVars)&
  ...
  !
  subroutine matrixMult(matLHS,solm1,vec_tmp,nVars,nn,i,j,k,l,m,n)
    use omp_lib
    implicit none
    integer  :: nVars
    Real*8   :: matLHS(nn,i,j,k,1:nVars,1:nVars)
    Real*8   :: solm1(l,m,n,1:nVars)
    Real*8   :: vec_tmp(1:nVars)
    integer  :: x, y, nn, i, j, k, l, m, n
    Real*8   :: s
    !$omp parallel &
    !$omp shared ( matLHS, solm1, vec_tmp, s, nn, i, j, k, l, m, n) &
    !$omp private (x, y)
    ...
    do x = 1, nVars
       s = 0.0
       do y = 1, nVars
         s = s + matLHS(nn,i,j,k,x,y) * solm1(l,m,n,y)
       end do
       vec_tmp(x) = s
    end do
  !$omp end do
